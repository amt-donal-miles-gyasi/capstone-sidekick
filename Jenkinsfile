...
def deploy_group = 'gitinspired-backend-dev'
def deploy_group_prod = 'gitinspired-backend-prod'
...
def main_branch = ['staging', 'develop', 'feat/subsys-refactor-GI-79,80,81']
...
// Successful Build
def buildSuccess = [
  [text: "Gitinspired Backend Build Successful on ${branchName}",
  fallback: "Gitinspired Backend Build Successful on ${branchName}",
  color: "#00FF00"
  ]
]
...
// Failed Build
def buildError = [
  [text: "Gitinspired Backend Build Failed on ${branchName}",
  fallback: "Gitinspired Backend Build Failed on ${branchName}",
  color: "#FF0000"
  ]
]
...
pipeline {
 ...
 post {
    ...
    success {
      ...
      withAWS(region:'eu-west-1',credentials:'aws-cred') {
        sh 'aws ses send-email --from cloudamalitech@amalitech.org --to caleb.osam@amalitech.com --subject "Deployment passed" --text "Gitinspied Backend Deployment passed"'
      		}
    }
    unstable {
      echo 'I am unstable :/'
    ...
    script {
      ...
      withAWS(region:'eu-west-1',credentials:'aws-cred') {
        sh 'aws ses send-email --from cloudamalitech@amalitech.org --to caleb.osam@amalitech.com --subject "Deployment failed" --text "Gitinspied Backend Deployment failed"'
      		}
    }
    changed {
