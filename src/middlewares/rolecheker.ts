...
/**
 * Middleware to check if a user is a lecturer.
 * If the user is authenticated and has the role of 'LECTURER', calls the next middleware; otherwise, returns a forbidden error.
 *
 * @param {Request} req - The Express Request object.
 * @param {Response} res - The Express Response object.
 * @param {NextFunction} next - The next middleware function.
...
export const isLecturer = (req: Request, res: Response, next: NextFunction) => {
  if (req.isAuthenticated() && (req.user as User).role === 'LECTURER') {
    return next();
  }
